...
function ExpressHandlebars (config) {
	...
	if (this.aws) {
		var credential = {
			accessKeyId: this.aws.accessKeyId || process.env.AWS_ACCESS_KEY_ID,
			secretAccessKey: this.aws.secretAccessKey || process.env.AWS_SECRET_ACCESS_KEY,
			region: this.aws.region || process.env.AWS_DEFAULT_REGION || "ap-northeast-2",
			signatureVersion: this.aws.signatureVersion || "v4",
			// signatureCache: this.aws.credential.signatureCache || false,
		};

		var s3 = new AWS.S3(credential);

		this.aws = {
			s3: s3,
			s3Bucket: this.aws.s3Bucket,
			s3Prefix: this.aws.s3Prefix,
			// viewsDir: this.aws.viewsDir,
		};
	}
}
...
ExpressHandlebars.prototype._getFile = function (filePath, options) {
	...
	file = cache[filePath] = new Promise(function (resolve, reject) {
		if (aws) {
			var regex = new RegExp(`(.*)/(${partialsDir.join("|")}|${viewsDir})(.*)`);
			var match = filePath.match(regex);
			// var _filePath = match[2];

			var keyPath = `${aws.s3Prefix}/${match[2]}${match[3]}`;
			aws.s3.getObject({ Bucket: aws.s3Bucket, Key: keyPath }, function (err, data) {
				if (err) {
					reject(err);
				} else {
